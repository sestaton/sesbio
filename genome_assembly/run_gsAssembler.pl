...
#check for prefix
if ($prefix) {
    print "============= Generating a subsample of reads for project name: ",
	$prefix, "\n" if $sample;
    print "============= Generating assembly for project name: ",
	$prefix, "\n" if !$sample;
}
else {
...
if ($sample) {
	...
	if ($num_subsamples) {
		print "============= Generating ", $num_subsamples,
	...
	if ( $bases && $reads ) {
		...
		print "============= Generating subsamples of bases for: ", $sample_cov,
	  	" bases ", "\n\n";
	}
	elsif ($reads) {
		print "============= Generating subsamples of reads for: ", $sample_cov,
	  	" reads ", "\n\n";
	}
}
...
if ($sample) {
	...
	for ( $it = 1; $it <= $num_subsamples; $it++ ) {
		...
		#likely other ways to reference something like this, will explore...
		my $its = ($it);
...
chdir("..");
opendir( RESULTSDIR, $results_dir ) || die "Can't open directory: $results_dir";
my @sub_sffs =
  map { "$results_path/$_" } grep { $_ !~ /^\./ } readdir(RESULTSDIR);
closedir(RESULTSDIR);
...
sub get_NewblerMetrics {
	...
	my ( $infile, $out, $parsed ) = @_;
	$parsed .= "_parsed.txt";
	open( IN,  $infile ) || die "\nERROR: Could not open file: $infile\n";
	open( OUT, ">$out" ) || die "\nERROR: Could not open file: $outfile\n";

	print OUT join("\t",("#filename","num_reads","num_bases","tot_num_reads","tot_num_bases",
	...
	close(IN);
	close(OUT);
	copy("$out", "$parsed") || die "Copy failed: $!";
