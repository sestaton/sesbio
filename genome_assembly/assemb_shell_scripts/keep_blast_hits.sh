...
grep ">" $1 | sed 's/>//' | sort > $sorted_fasta_ids
  
echo "Capturing IDs for $2 ..."
cut -f 1 $2 | sort -u > $sorted_blast_ids
...
comm -2 $sorted_blast_ids $sorted_fasta_ids > $comp_id_list

filteredct=$(awk 'END{print NR}' $comp_id_list)
sumct=$(($fastact+$blastct))
nonhitct=$(($fastact-$blastct))
...
log Total discarded reads: $nonhitct
log Total filtered reads: $filteredct

echo "Total num reads: $sumct"
echo "Total discarded reads: $nonhitct"
