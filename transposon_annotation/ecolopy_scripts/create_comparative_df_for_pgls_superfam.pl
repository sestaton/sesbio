...
open my $out, '>', $outfile;
my $sfam_ct = scalar keys %sph;
print $out "Species\tGenomeSize\tTotRepFrac\t", join "\t", (sort keys %sph), "\n";
keys %sph;
...
for my $species (sort keys %df) {
    ...
    for my $sf (sort keys %sph) {
	my $key = mk_key($sf, $species);
	if (exists $df{$species}{$sf} && exists $cvalh->{$species}) {
	    my $bpsize = $df{$species}{$sf} * $cvalh->{$species};
