...
## TODO: Print family of all TEs to standarize the output format
##       Sort annotations based on read number (cluster size)
##
##      
...
###//////////////////////////////////
#my $community = louvain_method($int_path, $outdir); ##// for debugging community
#my $cls_file = make_clusters($community, $int_path, $idx_path, $outdir); ##// for debugging community
###//////////////////////////////////
...
my ($cls_dir_path, $cls_with_merges_path, $cls_tot) = merge_clusters($infile, $str, $vertex, $seqs, 
								     $read_pairs, $report, $cluster_size, $outdir);
...
my ($anno_rp_path, $anno_sum_rep_path, $total_readct,                                                                           
    $rep_frac, $blasts, $superfams) = annotate_clusters($cls_dir_path, $database, $json, $report, 
							$outdir, $evalue, $seqct, $cls_tot);

### Generate whole-genome summary statistics
...
sub blast2annot {
    my ($json, $filebase, $readct, $top_hit, $top_hit_perc) = @_;
	...
	for my $class (keys %{$repeats->{$type}}) {
			...
			#$cluster_annot{$readct} = $anno_key;
				    ...
				    my $unk_fam = " ";
				    #try {
					my $anno_key = mk_key($filebase, $type, $class, $superfam_h, $unk_fam, $$top_hit, $$top_hit_perc);
					$cluster_annot{$readct} = $anno_key;
				    #}
				    #catch {
