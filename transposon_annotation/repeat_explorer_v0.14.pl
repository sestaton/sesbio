...
sub annotate_clusters {
    my ($cls_with_merges_dir, $database, $json, $report, $outdir, $evalue) = @_;
...
sub clusters_annotation_to_summary  {
    ...
    while (<$in>) {
        ...
        elsif (scalar @fields == 6) { ## all TEs but SINEs
            my $fam = $fields[5];
            $fam =~ s/\-\d.*//;
            $sfam_hitct{$fields[4]}++;
	    ...
	    #say join "\t", @fields;  # for debug
	    #say scalar(@fields)," fields.";
	    #next;
	    my $fam = $fields[6];
            $fam =~ s/\-\d.*//;
            $sfam_hitct{$fields[5]}++;
    ...
    close $in;
    say $outsum join "\t", "Superfamily", "Family", "ReadCt/TotalReadCt", "ReadCt/AllClustersReadCt", "PerCov", "AllPerCov";
	...
	for my $f (reverse sort { $fam_readct{$a} <=> $fam_readct{$b} } keys %fam_readct) {
		...
		my $all_perc_cov = sprintf("%.12f",$read_ct/$total_readct);
                say $outsum join "\t", $sf, $f, $read_ct."/".$total_ct, $read_ct."/".$total_readct,$perc_cov,$all_perc_cov;
	    }
	}
    }
    close $outsum;
