...
unless (defined $hmm_path) {
    $hmm_path = "/usr/local/hmmer/hmmer-2.3.2";    ## Set a default path for HMMER. Since we want to build global models, set to hmmer-2.3.2.
}
...
# from batch_genscan ## probably a more sophisticated way of doing things. 
my $date = `date`;
chomp $date;
$date =~ s/\s+/\_/g;
...
# make one directory
my $model_dir = "LTR_hmmer2_models-".$date;
unless ( -e $model_dir ) {
...
foreach my $LTR_aln (@LTR_aln_files) {
...
opendir( HMMS, "../$model_dir") || die "ERROR: Could not open directory: $model_dir\n";
...
my $hmmresults = "hmmsearch_out-".$date;
unless ( -e $hmmresults ) {
    mkdir($hmmresults);
}
chdir($hmmresults);

my $hmmresults_parsed = "hmmsearch_out_parsed-".$date;
unless ( -e $hmmresults_parsed ) {
    mkdir($hmmresults_parsed);
}

foreach my $hmm (@LTR_hmm_files) {
...
sub get_files {
    ...
    my $dir = shift;
    opendir( DIR, $dir ); # || print "Can't open directory:\n$in_dir\n";
    my @dir_contents = grep /\.aln$|\.fa$|\.fas|\.fasta$/, readdir DIR ;                            # we are working with the .fasta ending
    closedir( DIR );
...
sub write_hmmsearch_report {
    ...
    open(OUTFILE,">$parsed") || die "Could not open file: $parsed\n";
    ...
    if (defined $header) {
	print OUTFILE join("\t", ("#query", "query_length", "number_of_hits", "hit_name", 
				  "perc_coverage","hsp_length", "hsp_perc_ident","hsp_query_start", "hsp_query_end", 
				  "hsp_hit_start", "hsp_hit_end","search_type")), "\n";
    }
    
    open(ALNIN, "../../../$ID_length_report") || die "Could not open file: $ID_length_report\n";
	...
	while( my $hit = $result->next_hit() ) {
		...
		while(<ALNIN>) {
			...
			if ( ($hsplen >= 80) && ($hsplen >= $aln_len * 0.80)  && ($hsplen == 0) ) { ## Need to revisit this $hsplen == 0 business
			    ...
			    print OUTFILE join("\t", ($query, $aln_len, $num_hits, $hitid, $percent_coverage,
	...
	if ( -d "../$seq_dir" ) {	   	    
	    foreach my $ext_seq (@ext_domain_seqs) {
		if ( -s $ext_seq ) {
