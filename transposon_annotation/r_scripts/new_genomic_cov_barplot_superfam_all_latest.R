...
# Ageratina
ager <- read.table("ager_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# H. annuus
ann1238 <- read.table("hann_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# H. arg
harg <- read.table("harg_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# H. port
hport <- read.table("hport_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# H. teph
hteph <- read.table("hteph_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# H. vert
hvert <- read.table("hvert_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# Phoeb
phoeb <- read.table("phoeb_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# CP
cp <- read.table("cp_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# Calyc
calyc <- read.table("calyc_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# Dasy
dasy <- read.table("dasy_all_annotationes_combined_summary.tsv", sep="\t", header=T)
...
# Gerb
gerb <- read.table("gerb_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# gnaph
gnaph <- read.table("gnaph_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# Saff
saff <- read.table("saff_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
# Senecio
sene <- read.table("sene_all_annotations_combined_summary.tsv", sep="\t", header=T)
sene.summ <- ddply(sene, "Superfamily", summarize, sum = sum(GenomePerc), as_perc = as_percent(GenomePerc), Cval = 4091, Mbp = sum * 4091)
...
# TKS
tks <- read.table("tks_all_annotations_combined_summary.tsv", sep="\t", header=T)
...
filtered_full_df <- fullcp[fullcp$GenomePerc >= 0.25,]

## plot

bp <- ggplot(filtered_full_df, aes(x=Species, y=Cval)) + 
geom_point(aes(size=Cval)) + 
labs(size="Genome size") + 
coord_flip() + 
ylab("Base pairs (millions)") + 
geom_bar(aes(x=Species, y=Mbp, fill=Superfamily), stat="identity",color="black") + 
...
scale_fill_manual(name="Superfamily",values = c("Gypsy" = "darkgreen","Copia" = "aquamarine4","DIRS" = "lightgreen",
					    "L1" = "darkgrey","EnSpm" = "azure2","Harbinger" = "azure4",
					    "Crypton" = "chartreuse","hAT" = "darkkhaki","Helitron" = "darkolivegreen3",
					    "Mariner/Tc1" = "chocolate","L2" = "aquamarine3","MuDR" = "dimgrey",
					    "ERV1" = "darkblue","CR1" = "chartreuse3","Penelope" = "cyan","R1" = "black")) + 
