...
my ($match_pairs, $match_index) = parse_mgbast($infile, $percent_id, $percent_cov);
...
my $cls_dir_path = clusters2fasta($cls_file, $seqhash);
...
USAGE: $script -i inreport -o hitsort -id 90.00 -cov 0.50 -f fasta_file
...
Options:
    -s|statsfile     :    A file to write alignment stats to. [NOT IMPLEMENTED]
