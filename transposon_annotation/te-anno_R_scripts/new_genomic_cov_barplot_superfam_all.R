...
bp <- ggplot(filtered_full_df, aes(x=Species, y=Cval)) + geom_point(aes(size=Cval)) + labs(size="Genome size") + coord_flip() + geom_bar(aes(x=Species, y=Mbp, fill=Superfamily), stat="identity",color="black") + scale_x_discrete(limits=c("Calyc","Dasy", "Gerb","Saff","TKS","CP","Sene","Gnaph","Ager", "Phoeb", "Harg","Hport","Hteph","Hvert","Hann"), labels=c("Nasanthus patagonicus",
                        "Fulcaldea stuessyi",
                        "Gerbera hybrida",
