...
Kstats_reorder <- read.table("all_15_species_superfam_multiphylosignal_stats_sig-only.txt",sep="\t",header=T)
rectangles <- data.frame( xmin = c(1,4,9), xmax = c(3, 8, 10), ymin = c(0, 0, 0), ymax=c(3.5, 3.5, 3.5), t=c("LTR-RT","non-LTR-RT","Class II"))
gp <- ggplot() + geom_point(data=Kstats_reorder, aes(Superfamily, K, color=Superfamily), size=4) + geom_rect(data=rectangles, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), fill=t, color="black", alpha=0.2, linetype=0)
                          
gp + scale_x_discrete(limits=c("Copia","ERV1","Gypsy","L1","L2","NeSL","Penelope","R1","Mariner/Tc1","hAT"),labels=c("Copia","ERV1","Gypsy","L1","L2","NeSL","Penelope","R1","Mariner/Tc1","hAT")) + scale_color_manual(name="Superfamily", values=c("Gypsy" = "darkgreen","Copia" = "aquamarine4","L1" = "darkgrey","hAT" = "darkkhaki","Mariner/Tc1" = "chocolate","L2" = "aquamarine","ERV1" = "darkblue","NeSL" = "chartreuse3","Penelope" = "cyan","R1" = "black")) + theme_bw()
