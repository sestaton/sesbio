...
bp <- ggplot(filtered_full_df, aes(x=Species, y=Cval)) + geom_point(aes(size=Cval)) + labs(size="Genome size") + coord_flip() + ylab("Base pairs (millions)") + geom_bar(aes(x=Species, y=Mbp, fill=Superfamily), stat="identity",color="black") + scale_x_discrete(limits=c("Calyc","Dasy", "Gerb","Saff","CP","TKS","Sene","Gnaph","Ager", "Phoeb", "Hport", "Hvert", "Hteph","Hann","Harg"), labels=c("Nasanthus patagonicus","Fulcaldea stuessyi","Gerbera hybrida","Carthamus tinctorius","Centrapallus pauciflorus","Taraxacum kok-saghyz","Senecio vulgaris","Pseudognaphalium helleri","Conoclinium coelestinum","Phoebanthus tenuifolius","Helianthus porteri","Helianthus verticillatus","Helianthus niveus ssp. tephrodes","Helianthus annuus","Helianthus arghophyllus")) + scale_fill_manual(name="Superfamily",values = c("Gypsy" = "darkgreen","Copia" = "aquamarine4","DIRS" = "lightgreen","L1" = "darkgrey","EnSpm" = "azure2","Harbinger" = "azure4","Crypton" = "chartreuse","hAT" = "darkkhaki","Helitron" = "darkolivegreen3","Mariner/Tc1" = "chocolate","L2" = "aquamarine3","MuDR" = "dimgrey","ERV1" = "darkblue","CR1" = "chartreuse3","Penelope" = "cyan","R1" = "black")) + theme(axis.text.x = element_text(color = "black", size = 12),axis.text.y = element_text(color = "black", size = 14), axis.title.x = element_text(color = "black", size = 14), axis.title.y = element_blank())
