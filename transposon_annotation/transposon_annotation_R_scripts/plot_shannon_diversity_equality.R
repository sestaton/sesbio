...
Aster_shannon_stats <- read.table("all_15_species_te-families_shannon_diversity_stats.txt",header=T,sep="\t")

ggplot(Aster_shannon_stats, aes(x=Species, y=H)) + 
  geom_point(size=3) + 
  theme_bw() + 
  coord_flip() + 
  ...
  geom_point(data=Aster_shannon_stats, aes(x=Species, y=J), size=3, color="blue") 