...
if ($search) {
	...
	print "\n========> Running Tallymer Search on sequence: $key\n" unless $quiet;
	my $oneseq = getFh($key);
	$matches = tallymer_search($oneseq, $index);
	if ($gff) {
	    foreach my $seqregion (sort keys %$seqreg) {
		if ($key eq $seqregion) {
...
sub split_mfasta {
    ...
    if ($seqct > 1) {
	print "\n========> Running Tallymer Search on $seqct sequences.\n" unless $quiet;
    } 
...
sub tallymer_search {
    ...
    #print "\n========> Outfile is $searchout\n" unless $quiet;    # The Tallymer search output. 
    system($search);
    return($searchout);
...
sub tallymersearch2gff {
    ...
    print $gff "##sequence-region ",$seqid," 1 ",$end,"\n";
...
sub printgff {
    ...
    print $gff join("\t",
		    ($seqid, "Tallymer", "MDR", $offset, $offset, $count, ".", "+",
		     join(";",
			  ("Name=Tallymer_".$merlen."_mer","ID=$seq","dbxref=SO:0000657")
			  )
		     )
		    ),"\n";
